 ...
 */
public class PerfMockery implements SelfDescribing {
    ...
    private final PerfInvocationDispatcher dispatcher = new PerfInvocationDispatcher(new CopyOnWriteArrayList(), new CopyOnWriteArrayList());
    ...
    public <T> T mock(Class<T> typeToMock, PerfModel model) {
        String name = namingScheme.defaultNameFor(typeToMock);
        return mock(typeToMock, name, model);
    }

    public <T> T mock(Class<T> typeToMock, String name, PerfModel model) {
        T obj = mock(typeToMock, name);
        dispatcher.addPerfModel(name, model);
        return obj;
    }
     ...
     */
    public PerfSequence sequence(String name) {
        return new PerfNamedSequence(name);
    }
    ...
    public void repeat(int times, final Runnable scenario) {
